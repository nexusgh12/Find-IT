...
public class DataController {
	...
	public ResponseEntity<String> dataWirteOk(DataVO vo, HttpServletRequest request) {
		...
		try {
			...
			//레코드 추가 성공
			String msg = "<script>alert('제품정보가 등록되었습니다');location.href='/myapp/data/dataList';</script>";
			...
			//메세지
			String msg = "<script>alert('제품등록을 실패하였습니다');history.back();</script>";
			//이전페이지로 보내기
	...
	public ResponseEntity<String> editOk(DataVO vo, HttpSession session, HttpServletRequest req) {
		...
		try {
			...
			// 글 내용보기로 이동
			String msg = "<script>alert('제품정보가 수정되었습니다.\\n제품 정보 보기로 이동합니다');";
	...
	//자료실 삭제
	   ...
	   public ResponseEntity<String> dataDel(int no, HttpSession session) {
	      ...
	      try {
	         ...
	         String msg = "<script>alert('제품 정보가 삭제 되었습니다.');location.href='/myapp/data/dataList';</script>";
	         entity = new ResponseEntity<String>(msg, headers, HttpStatus.OK);
	      }catch(Exception e) {
	         e.printStackTrace();
	         
	         String msg = "<script>alert('제품 정보 삭제를 실패하였습니다.');history.back();</script>";
	         entity = new ResponseEntity<String>(msg, headers, HttpStatus.BAD_REQUEST);
	      }
	      return entity;
